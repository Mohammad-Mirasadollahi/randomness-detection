...
**Pass:** `QUALITY CHECK: PASS` — F1 beats all baselines and ROC-AUC ≥ 0.95.
...
**Example numbers:** core_natural FP **5%**, clear_random FN **0%**, dictionary-DGA (4+ words) recall **100%**.

---

### Ensemble integration test

**Script:** `test_ensemble.py`

**Question:** Does the full pipeline (LM + PMI + gradient boosting) pass smoke, quality, robustness, and dictionary-DGA checks?

**Pass:** `ENSEMBLE TEST: PASS`

**Ablation:** `ablation_benchmark.py` — remove LM, PMI, or lexical feature groups one at a time.
...
| `ablation_benchmark.py` | Feature-group ablation study |
