...
|--------|--------|----------|-----|---------|----------------|---------------|-----------|
| **randomness_detection** | product | **100.0%** | **1.000** | **1.000** | **0.0%** | **0.0%** | 19 |
...
Entropy, compression, and standalone DEFLATE cannot separate short natural English from random strings.
...
|--------|--------|--------|
| core_natural (words, identifiers, phrases, digit-names) | false-positive rate | **5%** |
| clear_random (UUIDs, SHAs, base64, API keys, DGA) | false-negative rate | **0%** |
| dictionary-DGA (4+ words, curated) | recall | **100%** |

Also run `test_ensemble.py` for a combined smoke + quality + robustness + DGA check.
Ablation: `ablation_benchmark.py` — remove LM, PMI, or lexical groups one at a time.
...
7. **Character language model** — n-gram perplexity (natural vs random plausibility)
8. **Word PMI** — co-occurrence plausibility of segmented dictionary words

A **gradient-boosting ensemble** with Platt calibration merges these into a single **1–100 score**.
...
│   ├── ngram_lm.py          # Character language model
│   ├── pmi_model.py         # Word PMI model
│   ├── ensemble_features.py # Full feature vector
