"""Train and persist the LRD-Hybrid gradient-boosted ensemble."""
...
from .hybrid_features import FEATURE_GROUPS, HybridFeatureVector
from .io_utils import atomic_pickle_dump


class HybridEnsembleModel:
    """Calibrated gradient-boosting classifier over hybrid features."""
    ...
    def predict_random_probability(self, features: HybridFeatureVector) -> float:
    ...
    @classmethod
    def load(cls, path: Path) -> "HybridEnsembleModel":
...
def train_hybrid_ensemble(
    feature_rows: list[HybridFeatureVector],
    labels: list[int],
    *,
    active_groups: frozenset[str] | None = None,
    test_size: float = 0.2,
...
) -> tuple[HybridEnsembleModel, dict[str, float | int | str]]:
